...
async def key_issued_kb(
    ...
    *,
    ...
    ``key_url`` is accepted as a kwarg for forward-compat (previously used to
    render an "Импортировать" button) but is currently ignored — Telegram
    URL-buttons can't trigger the OS-level import handoff to Happ / V2RayTun
    reliably, so the button was removed.
    """
    del key_url  # see docstring — kept in signature to avoid call-site churn.
